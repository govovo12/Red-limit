...
def ws_connection_flow(task_list: List[Dict], max_concurrency: int = 1) -> List[int]:
    ...
    def handle_single_task(task: Dict) -> int:
        ...
        # Step 5: keep_alive
        print_info(f"[Step 5] 🚀 發送 keep_alive 小包：oid={oid}")
        ...
        # Step 6: send_bet
        print_info(f"[Step 6] 🚀 發送 bet 小包：oid={oid}")
        ...
        print_info(f"[Step 7] 🎯 發送 cur_round_finished 小包：oid={oid}")
        ...
        # Step 8: exit_room
        print_info(f"[Step 8] 🚪 發送 exit_room 小包：oid={oid}")
    ...
    if error_records:
        ...
        for err in error_records:
            print(f"🔻 code={err['code']} step={err['step']} account={err['account']} game={err['game_name']}")
